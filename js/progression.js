  ...
  screen.innerHTML = `
    ...
    <div class="progression-content">
      ...
      <div class="stats-row">
        ...
        <div class="stat-card">
          <span class="stat-value" id="stat-reflect">—</span>
          <span class="stat-label" id="stat-reflect-label">Reflected 🌱</span>
        </div>
      </div>
      ...
      <div class="chart-card">
        <div class="chart-header">
          <div>
            <h3>Mood Trend</h3>
            <p class="chart-hint">Tap a dot to open that entry</p>
          </div>
...
function updateProgressionData(entries) {
  ...
  const negEntries     = entries.filter(e => {
    if (!e.emotions || !e.emotions.length) return false;
    const scores = e.emotions.map(id => EMOTIONS.find(x => x.id === id)?.score ?? 0);
    return scores.reduce((a, b) => a + b, 0) / scores.length < 0;
  });
  const reflectedCount = negEntries.filter(e => e.reframe && e.reframe.trim()).length;
  const reflectEl      = document.getElementById('stat-reflect');
  const reflectLblEl   = document.getElementById('stat-reflect-label');
  if (reflectEl) {
    if (!negEntries.length) {
      reflectEl.textContent      = '—';
      reflectLblEl.textContent   = 'No lows yet';
    } else {
      reflectEl.textContent      = `${reflectedCount}/${negEntries.length}`;
      reflectLblEl.textContent   = 'Reflected 🌱';
    }
  }
  ...
  const points = entries
    ...
    .map(e => {
      ...
      const label  = e.emotions
        ...
        .join('  ');
      return { score: avg, emotions: e.emotions, label, entryId: e.id };
    });
...
function drawMoodTrend(canvas, points) {
  ...
  points.forEach((pt, i) => {
    ...
    dotPositions.push({ x, y, label: pt.label, entryId: pt.entryId });
  });
...
function setupTrendTooltip(canvas, tooltip) {
  ...
  if (_trendHandlers) {
    ...
    canvas.removeEventListener('click',      _trendHandlers.click);
  }
  ...
  _trendHandlers = {
    ...
    leave: ()  => { tooltip.hidden = true; canvas.style.cursor = 'default'; },
    ...
    click: e  => {
      ...
      if (nearest && nearest.entryId != null) {
        setExpandOnRender(nearest.entryId);
        navigateTo('history');
      }
    },
  };
  ...
  canvas.addEventListener('click',      _trendHandlers.click);
}
