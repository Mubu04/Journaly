...
/* ── Structured entry text helpers (shared across screens) ── */

/**
 * Combines the 3 journal sections into one stored string.
 * Sections that are empty are omitted so old entries with fewer sections still parse cleanly.
 */
function buildEntryText(what, why, realise) {
  const parts = [];
  if (what)    parts.push(`What happened:\n${what}`);
  if (why)     parts.push(`Why I felt that way:\n${why}`);
  if (realise) parts.push(`What I realised:\n${realise}`);
  return parts.join('\n\n');
}

/**
 * Parses a stored entry text back into its 3 sections.
 */
function parseEntryText(text) {
  if (!text) return { what: '', why: '', realise: '' };

  const WHAT    = 'What happened:\n';
  const WHY     = 'Why I felt that way:\n';
  const REALISE = 'What I realised:\n';

  const whatIdx    = text.indexOf(WHAT);
  const whyIdx     = text.indexOf(WHY);
  const realiseIdx = text.indexOf(REALISE);

  /* Old-format entry — no section markers present */
  if (whatIdx === -1 && whyIdx === -1 && realiseIdx === -1) {
    return { what: text, why: '', realise: '' };
  }

  const out = { what: '', why: '', realise: '' };

  if (whatIdx !== -1) {
    const start = whatIdx + WHAT.length;
    const end   = whyIdx !== -1 ? whyIdx - 2 : (realiseIdx !== -1 ? realiseIdx - 2 : text.length);
    out.what = text.slice(start, Math.max(start, end)).trim();
  }
  if (whyIdx !== -1) {
    const start = whyIdx + WHY.length;
    const end   = realiseIdx !== -1 ? realiseIdx - 2 : text.length;
    out.why = text.slice(start, Math.max(start, end)).trim();
  }
  if (realiseIdx !== -1) {
    out.realise = text.slice(realiseIdx + REALISE.length).trim();
  }
